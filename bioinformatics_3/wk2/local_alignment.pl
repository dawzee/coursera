...
sub LoadScoringMatrix
{
    open(my $fh, "<PAM250_1.txt" ) || die "Can't open PAM250_!.txt: $!";
    my @raw = <$fh>;
    close($fh);
...
sub LCSBacktrack
{
    ...
    {
        $longest[0][$col_index] = 0;
        $backtrack[0][$col_index] = 'f';
    }
    ...
    for(my $row_index = 1; $row_index <= $#in_str1_idx+1; $row_index++)
    {
        for(my $col_index = 1; $col_index <= $#in_str2_idx+1; $col_index++)
        {
            ...
            # check if a free ride would have been easier
            if($longest[$row_index][$col_index] < 0)
            {
                $longest[$row_index][$col_index] = 0;
                $backtrack[$row_index][$col_index] = 'f';
            }
            
            # keep track of max score position
            if($longest[$row_index][$col_index] > $longest[$max_row][$max_col])
            {
                $max_row = $row_index;
                $max_col = $col_index;
            }
